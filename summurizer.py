...
class VideoSummarizer:
    def __init__(self, model_name):
        ...
        self.model = AutoModelForSeq2SeqLM.from_pretrained(model_name, torch_dtype=torch.float).to(device)
        self.model.eval()

    def summarize(self, video_path: str, max_new_tokens: int) -> None:
        input = preprocess_video(video_path, model = self.model_whisper)
        summary = generate_summary(input, self.model, self.tokenizer, max_new_tokens) 
        return summary
